...
import { GetCareRecordResponsDto, GetCustomerListResponseDto, GetCustomerResponseDto } from "./dto/response/customer";
...
const PATCH_CUSTOMER_API_URL = (customerNumber: number | string) => `${CUSTOMER_MODUEL_URL}/${customerNumber}`;
const DELETE_CUSTOMER_API_URL = (customerNumber: number | string) => `${CUSTOMER_MODUEL_URL}/${customerNumber}`;
const POST_CARE_RECORD_API_URL = (customerNumber: number | string) => `${CUSTOMER_MODUEL_URL}/${customerNumber}/care-record`;
const GET_CARE_RECORD_LIST_API_URL = (customerNumber: number | string) => `${CUSTOMER_MODUEL_URL}/${customerNumber}/care-records`;
...
export const getCareRecordListRequest = async (customerNumber: number | string, accessToken: string) => {
    const responseBody = await axios.get(GET_CARE_RECORD_LIST_API_URL(customerNumber), bearerAuthorization(accessToken))
        .then(responseDataHandler<GetCareRecordResponsDto>)
        .catch(responseErrorHandler);
    return responseBody;
};
